...
import fr.mqrtin.utility.module.impl.property.properties.ColorProperty;
...
public class ColorSliderComponent implements Component {
    ...
    public void draw(java.util.concurrent.atomic.AtomicInteger offset) {
        ...
        Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(property.getName().replace("-", " ") + ": " + String.format("#%06X", property.getValue()), (float) (x * 2), (float) ((int) ((float) (this.parentModule.category.getY() + this.offsetY + 3) * 2.0F)), -1);
        GL11.glPopMatrix();
    ...
    public void update(int mouseX, int mouseY) {
        ...
        if (changed) {
            ...
            parentModule.mod.verifyValue(property.getName());
        }
    }
