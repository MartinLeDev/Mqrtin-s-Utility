...
public class ModuleComponent implements Component {
    ...
    public ModuleComponent(Module mod, CategoryComponent category, int offsetY) {
        ...
        int y = offsetY + 12;
        ArrayList<Property<?>> properties = Main.propertyManager.properties.get(mod.getClass());
        if (properties != null && !properties.isEmpty()) {
            for (Property<?> baseProperty : properties) {
                if (baseProperty instanceof BooleanProperty) {
                    ...
                    SliderComponent c = new SliderComponent(property, this, y);
                    this.settings.add(c);
                    y += c.getHeight();
    ...
    public void draw(AtomicInteger offset) {
        ...
        if (this.mod.isEnabled()) {
            textColor = 0xFF00FF00; // Vert pour activé
        } else {
            textColor = 0xFF666666; // Gris pour désactivé
        }
        Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(this.mod.getModuleName(), (float) (this.category.getX() + this.category.getWidth() / 2 - Minecraft.getMinecraft().fontRendererObj.getStringWidth(this.mod.getModuleName()) / 2), (float) (this.category.getY() + this.offsetY + 4), textColor);
