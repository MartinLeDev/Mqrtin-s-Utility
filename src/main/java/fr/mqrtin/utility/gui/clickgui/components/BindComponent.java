...
public class BindComponent implements Component {
    ...
    public void draw(AtomicInteger offset) {
        ...
        String keyName = this.parentModule.mod.getKeybind() != null ? Keyboard.getKeyName(this.parentModule.mod.getKeybind().getKeyCode()) : "NONE";
        String displayText = this.isBinding ? "Press key..." : "Bind: " + keyName;
        this.renderText(displayText, 0xFF00FF00);
        GL11.glPopMatrix();
    }
    ...
    public void keyTyped(char chatTyped, int keyCode) {
        if (this.isBinding) {
            ...
            if (this.parentModule.mod.getKeybind() != null) {
                // Créer un nouveau KeyBinding avec le code de touche spécifié
                KeyBinding oldBinding = this.parentModule.mod.getKeybind();
                KeyBinding newKeyBinding = new KeyBinding(oldBinding.getKeyDescription(), keyCode, oldBinding.getKeyCategory());
                this.parentModule.mod.setKeybind(newKeyBinding);
            }
